...
- 修复首页 Ping 完整弹窗与节点详情页关闭“丢包数据”后，单图时间刻度重叠的问题：基础时间轴启用与双图一致的标签避让，不改变字体、时间轴类型、采样数据或查询范围。
- 增加实际 ECharts 模型与 Canvas 标籤位置回归，覆盖窄屏、连续切换、长时间窗、选择／图例、旋转、空数据、零值与真实丢包语义。
- 增加固定 Komari 1.5.1 官方二进制的隔离安装／升级验证，并保留 fix1、1.5.0、1.4.3 对照；具体平台与结果见[兼容记录](docs/compatibility/komari-1.5.1.md)。未移植官方前端或改动后台。
- 保留 Header Tooltip、Ping 圆点标签、页脚、配置与同地址 `/sw.js` 在线兼容策略；不修改依赖或发布其他版本。

## [2.8.3] - 2026-09-21（正式发布）
...
- 沿用兼容 `/sw.js` 和在线访问策略，不恢复手动恢复页或官方离线外壳；安装包原样附带既有 LICENSE。新简介的[市场跟进资料](docs/theme-market-submission.md)供维护者更新既有条目。
- 同版本修复：恢复顶部工具按钮的美化提示，深色模式使用浅底深字及同色箭头，移除重复的原生 title；保留三种直接主题模式、按钮行为、无障碍名称和其他数据提示。
...
- 根作用域前台及后台不再使用原 Worker 的离线外壳和运行时缓存。在线升级、后台与资料保护测试详见[兼容说明](docs/compatibility/service-worker.md)；用户反馈当前使用正常，原 iPhone Safari 各模式仍未完整实测，不保证所有黑屏原因相同。
...
- 首页节点卡片新增公开备注标签，与在线时长、价格保持同款样式；仅展示公开备注纯文字，空值隐藏、长文本单行截断，不读取或回退到私有备注。
- 修复移动端共享 Tooltip 滚动时可能被兼容 mouseleave 提前关闭的问题，保留轻点关闭与桌面鼠标交互。
- 本版本为 Pre-release，待用户实际测试确认后再转为正式版本；保持既有 Ping 数据读取、缓存与调度行为。

## [2.8.0] - 2026-09-09

- 节点详情页与首页完整 Ping 弹窗默认开启丢包历史图，共用任务、时间范围、底部图例、时间参考线与单个 Tooltip；关闭“丢包数据”恢复单延迟图，首页小型时间桶与列表展示不变。
...
- 优化延迟／丢包共享 Tooltip 宽度，根据任务名称和数值自然排版，减少多余空白。
- 为探测不可达和非零丢包数值增加红色文字提示，极小正丢包显示为 `<0.1%`，缺失值保持未知。
- 手机端支持轻点 Tooltip 内容关闭，同时保留长列表上下滑动与滚动浏览。
- 撤销移动端 Tooltip 时间线避让与图外资料区，恢复此前悬浮提示行为；保留首页完整 Ping 弹窗双图、紧凑宽度、异常红字及手机轻点／滚动操作。
